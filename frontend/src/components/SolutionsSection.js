      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-0 lg:grid-cols-2 lg:gap-0">
        <div className="flex min-h-0 flex-col justify-between p-8 sm:p-10 lg:p-12 lg:pr-10 bg-[#FAFBFC] border-r border-slate-100">
          <div className="min-h-0 flex-1 overflow-y-auto lg:overflow-visible">
            <h3 className="mb-1 text-[#172B4D] text-2xl font-bold tracking-tight">{solution.title}</h3>
            {solution.cardDescriptor ? (
              <p className="mb-4 text-xs font-medium uppercase tracking-wide text-[#6B778C]">{solution.cardDescriptor}</p>
            ) : null}
...
export default function SolutionsSection({ showLabel = true }) {
  ...
  return (
    <section id="solutions" data-testid="solutions-section" className="overflow-hidden bg-white text-slate-900 border-b border-slate-100">
