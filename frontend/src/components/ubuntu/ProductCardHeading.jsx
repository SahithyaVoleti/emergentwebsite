  ...
  return (
    <div className={`ubuntu-product-card__heading flex items-stretch justify-start gap-3 px-3 sm:px-4 ${className}`}>
      ...
      <div className="ubuntu-product-card__heading-copy min-w-0 flex-1">
        <h3 className="ubuntu-product-card__heading-text text-base font-medium text-[#2d2d2d] leading-snug sm:text-lg">
          {children}
        </h3>
        {descriptor ? (
          <p className="ubuntu-product-card__heading-desc mt-1 text-xs font-normal leading-snug text-[#6b6b6b] line-clamp-2 sm:text-sm">
            {descriptor}
          </p>
        ) : null}
      </div>
